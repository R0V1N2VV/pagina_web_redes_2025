...
// Consulta principal filtrada por tipo = 'bolsa'
...
// Plan A: Obtener historial de los últimos 5 minutos (filtrando por tipo 'bolsa')
$histStmt_5min = $pdo->prepare("
    SELECT ph.precio 
    FROM precios_historial ph 
    JOIN precios p ON ph.simbolo = p.simbolo
    WHERE ph.simbolo = :s AND p.tipo = 'bolsa' AND ph.fecha >= DATE_SUB(NOW(), INTERVAL 5 MINUTE)
    ORDER BY ph.fecha ASC
");

// Plan B (Fallback): Obtener los últimos 10 puntos si los 5 minutos están vacíos (filtrando por tipo 'bolsa')
$histStmt_fallback = $pdo->prepare("
    SELECT ph.precio 
    FROM precios_historial ph 
    JOIN precios p ON ph.simbolo = p.simbolo
    WHERE ph.simbolo = :s AND p.tipo = 'bolsa'
...
<?php
// Código del navbar insertado directamente (sin include)
...
<div class="container">
<h2 class="text-center text-xl font-semibold text-purple-400 mb-4">Bolsa de Valores</h2>
